...
import { app_vertical, link_style } from "@/lib/variables/variables";
import { roboto } from "@/styles/fonts";
import { IOrder } from "@/types/interfaces/order/order";
import { AppProductSummary, AppTypography } from "@/components/shared";
    ...
    return (
        <main className="container flex items-start justify-between gap-12 pt-20">
            <div className={cn(app_vertical, "min-w-[70%] gap-6")}>
                <OrderInformationLayout
                    ...
                    rows={[
                        ...
                        { key: "Transaction ID", value: <a className={cn(roboto.className, "font-medium text-sm text-[#179EF8] underline cursor-pointer")} target="_blank" href={deployHashModel.deploy_hash_link(order?.details?.deployHash || "", order?.details?.chain)}>{order?.details?.deployHash}</a>, condition: Boolean(order?.details?.deployHash ?? false) && Boolean(order?.details?.deployHash) && Boolean(order?.details?.chain) },
            ...
            <div className="min-w-[30%] sticky left-0 top-24">
                ...
                <div className={cn(app_vertical, "gap-6 w-full")}>{order?.items?.map(({ _id, size, color, quantity, price, image, title }) => <AppProductSummary key={_id} details={{ size, color, quantity, priceItem: price, image, title }} />)}</div>
            </div>
        </main>
    );
}
