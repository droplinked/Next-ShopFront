    ...
    return (
        <form onSubmit={handleSubmit} className={cn(app_vertical, 'gap-6')}>
            <div className={cn(app_vertical, "border rounded-sm p-6 gap-6 w-full")}>
                ...
                <div className={cn(app_center, "w-full gap-6")}>
                    <AppDropDownInput name="country"  value={values.country ? { label: values.country, value: values.country } : null} typing={(e: string) => {}} isLoading={false} select={_select_country} options={locations?.countries?.map((el: any) => ({ value: el.name, label: el.name }))} label="Country"/>
                    <AppDropDownInput name="city" value={values.city ? { label: values.city, value: values.city } : null} typing={(e: string) => setLocations((prev)=> ({ ...prev, search: e }))} isLoading={false} select={_select_city} label="City" options={locations.cities.length && locations?.cities?.map((el: any) => ({ value: el.name, label: el.name + ` (${el.state_name})` })) || []}/>
