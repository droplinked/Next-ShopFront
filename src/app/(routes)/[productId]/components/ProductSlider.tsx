        ...
        {media && media?.length
          ? media?.slice(0, 4).map((el: any, key: number) => (
              <div
                ...
                onClick={() => setImageSlider(el.url)}
              >
                {[media?.length - 1, 4 - 1]?.includes(key) && (
                  <div
                    {...bindTrigger(popupState)}
                    className="rounded-sm flex absolute top-0 bottom-0 left-0 right-0 items-center cursor-pointer justify-center text-center z-10 bg-[rgba(0,0,0,.6)]"
                  >
                    <AppTypography appClassName="text-white text-xs">
                      See More
                    </AppTypography>
                  </div>
                )}
