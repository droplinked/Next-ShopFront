...
import { AppButton, AppDialog, AppIconButton, AppSeparator, AppTypography } from "@/components/shared";
                                ...
                                then: (
                                    <footer className={cn(app_vertical, "w-full sticky bottom-0 pb-6 bg-background gap-6")}>
                                        <div className={cn(app_center, "justify-between w-full")}>
                                            ...
                                            <AppTypography price appClassName="text-xl font-medium">{cart?.totalCart?.subtotal}</AppTypography>
                                        </div>
                                        <div onSubmit={(e) => { e.preventDefault() }} className="flex justify-center items-center w-full gap-6">
                                            <div className="min-w-fit"><AppButton onClick={() => updateState({state: "cart", value: {}})} appClassName="rounded-sm w-full text-base font-normal" appVariant="outlined" appSize="md">Clear</AppButton></div>
