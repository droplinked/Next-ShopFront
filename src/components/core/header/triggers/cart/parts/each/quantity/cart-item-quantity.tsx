import { AppQuantity } from '@/components/ui';
...
const CartItemQuantity = ({ options: { quantity }, skuID, _id }: Pick<ICartItem, 'options' | 'skuID' | '_id'>) => {
  const { updateCartItemQuantity } = useAppCart();
  const { states: { cart }} = useAppStore();

  const _update_quantity = async (new_quantity: number) =>
    cart?._id &&
    _id &&
    toast.promise(async () => await updateCartItemQuantity({ cartId: cart._id, quantity: new_quantity, skuID, itemId: _id }), {
      loading: 'Changing item quantity...',
      success: 'Changing quantity',
      error: 'Got an error'
    });
