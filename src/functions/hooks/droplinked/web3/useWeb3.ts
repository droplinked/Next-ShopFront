...
function useWeb3Hook() {
  ...
  const login: ILoginDroplinked = {
    MULTIWALLET: {
      wallet: 'Multi wallet',
      chains: {
        UNSTOPPABLEDOMAIN: { caption: 'Unstoppabledomains', method: () => {}, icon: { dark: AppWalletIcons.UnstoppablePayment, light: AppWalletIcons.UnstoppablePayment } }
      }
    },
    XUMM: {
      wallet: 'Xumm',
      chains: { XUMM: { caption: 'XRPL', method: () => {}, icon: { dark: AppWalletIcons.XrplSidechainPayment, light: AppWalletIcons.XrplSidechainPayment } } }
    },
    METAMASK: {
      wallet: 'Metamask',
      chains: {
