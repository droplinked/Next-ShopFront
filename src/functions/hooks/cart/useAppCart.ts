...
function useAppCart() {
    ...
    const _add_params = async ({ skuID, quantity, m2m_data }: IAddToCart) => ({ cartId: cart._id ? cart._id : (await _create())?._id, shopID: shop?._id, skuID, quantity, ...(m2m_data && { m2m_data })})
    ...
    const add_gift = ({...params}: IApplyGiftCardService) => new Promise<any>(async (resolve, reject) => apply_giftcard_service({...params}).then((res) => resolve(_update(res))).catch((err => reject(err))));
    ...
    const change = async ({cartId, skuID, quantity, itemId}: IChangeQuantity) => new Promise<ICart>(async (resolve, reject) => await change_quantity_service({ cartId, shopID: shop?._id, skuID, quantity, itemId}).then((res) => resolve(_update(res))).catch(err => reject(err)));
